...
"深低位 + 贴底放量突破"——`low_flat_limit_up` 的右侧孪生版：股价深跌至窗口高点之下（默认回撤 ≥ 65%）且仍贴近窗口低点（低点距今 ≥ 40 根、离底 ≤ 50%），一根放量阳线（≥ 前 5 日均量 2 倍）收盘突破此前一个月平台高点 ≥ 2% 且此后未跌回平台内（默认 ≥ 2 根确认K线），同时 MA20 止跌走平、股价站上 MA20。刻意不含 `low_percentile` 闸门：突破收盘价按构造就处于底部区间分布的顶端，分位必然失效。
...
每个策略都是对可复用原子过滤器的声明式谓词，通过 AND / OR / NOT 组合成表达式树。当前内置十五个过滤器（`a_share_list_filters` 可列出各自的参数）：
...
- 行业分类来自共享股票列表的东财 `f100` 字段（东财自有行业口径，非申万一级/二级）；三个内置源均提供（`capabilities.industry` 为 true），`industry_clearance` 基于它做行业周期聚合。`--board` 板块选股则走另一条东财行业/概念板块成分接口。
