...
const amountLiquidityFilter = {
	...
	apply(ctx, params) {
		...
		const medianYi = (amounts.length % 2 === 1 ? amounts[mid] : (amounts[mid - 1] + amounts[mid]) / 2) / 1e8;
		return {
...
const industryClearanceFilter = {
	...
	apply(ctx, params) {
		...
		if (stats === void 0) return {
			passed: false,
			evidence: {
				industry: ctx.stock.industry ?? null,
				industryMedDrawdown: null,
				industryDeepShare: null,
...
const turnoverBandFilter = {
	...
	apply(ctx, params) {
		...
		const medianPct = (turnovers.length % 2 === 1 ? turnovers[mid] : (turnovers[mid - 1] + turnovers[mid]) / 2) * 100;
...
function cycleSummary(series) {
	...
	let below = 0;
	for (const value of window) if (value <= idx) below++;
	return {
		dd,
		pos: window.length > 0 ? below / window.length : 1
	};
}
...
async function aggregateIndustries(host, config, dataSource, stocks, startDate, today, signal, fetchedThisRun) {
	...
	host.log("info", `industry aggregation: ${stats.size} industries over ${scanned} members (${withoutIndustry} without classification)`);
	return stats;
}
...
const lowFlatBreakoutStrategy = composeStrategy({
	...
	description: "Deep low + volume breakout (right-side twin of low_flat_limit_up): the stock sits deep below its window high (default >= 65% drawdown, on the chained return index), and within the last ~2 weeks a volume-heavy day (default >= 2x the prior 5-day average) closed at least 2% above the high of the preceding one-month flat base with the price holding out of the base since, while the MA20 stopped falling and the price sits at or above it. Unlike low_flat_limit_up there is no latest-price percentile gate: a breakout close above the base high sits at the top of the bottom-side distribution by construction. Read the evidence fields as quantified facts, not trading signals.",
	predicate: {
		kind: "and",
		children: [
			{
...
*/
function customRegistry(deps, predicate) {
...
function createScreenTool(deps) {
	...
	return defineTool({
		...
		async execute(args, exec) {
			...
			let title;
			if (hasPredicate) {
				registry = customRegistry(deps, toPredicate(args.predicate, deps.filters));
